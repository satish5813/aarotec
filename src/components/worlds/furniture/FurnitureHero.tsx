...
/** CSS-driven line reveal: visible on first paint, no hydration needed. */
function Line({ children, delay }: { children: React.ReactNode; delay: number }) {
  return (
    <span className="mask-up !block">
      <span className="!block" style={{ "--d": `${delay}s` } as React.CSSProperties}>{children}</span>
    </span>
  );
}
    ...
    <section ref={ref} className="relative isolate overflow-hidden bg-bg pt-28 sm:pt-32">
      ...
      <div className="mx-auto grid max-w-[1400px] items-center gap-8 px-5 pb-16 sm:px-8 lg:min-h-[calc(100dvh-8rem)] lg:grid-cols-[0.9fr_1.1fr] lg:gap-10 lg:pb-20">
        ...
        <motion.div style={{ y: copyY, opacity: copyO }} className="relative z-10">
          <p className="rise label flex items-center gap-3 text-hi" style={riseAt(0.3)}>
          ...
          <p className="rise mt-7 max-w-lg text-lg leading-relaxed text-muted" style={riseAt(0.8)}>
          ...
          <div className="rise mt-8 flex flex-wrap gap-3" style={riseAt(1.05)}>
          ...
          <dl className="rise mt-10 grid max-w-md grid-cols-3 gap-6 border-t border-line pt-6" style={riseAt(1.2)}>
            {[
              { v: 6, s: "", l: "collections" },
